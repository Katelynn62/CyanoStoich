...
This figure is the map of our sites, and we have a separate R sheet "___" for this document. 
...
ggsave("MB_SA_PC.tiff", plot=Combined_PC, device="tiff", width=6, height=2.5, units="in", dpi=600)
```
## Cyanotoxin Levels: 
...
ggsave("MB_SA_Toxins.tiff", plot=Combined_Tox, device="tiff", width=6, height=2, units="in", dpi=600)
...
```
Still need a better solution to the bloom stages labels.

# Manuscript Figure 3: Phytoplankton Genera

I did a really bad job at working in the multiple different sheets I have for this project. 

Below is how I loaded and altered the data for different groups. 
...
* Make group names consistent. "phyta" vs. "phyceae"
...
ggsave("SA_PhytoGroup.tiff", plot=SA_21, device="tiff", width=4, height=3, units="in", dpi=600)
...
ggsave("MB_PhytoGroup.tiff", plot=MB_21, device="tiff", width=4, height=3, units="in", dpi=600)
...
ggsave("MB_SA_PhytoGroup.tiff", plot=Combined_PhytoGroup, device="tiff", width=8, height=3, units="in", dpi=600)
...
The main changes here are that we are going to increase the text size, make sure that the phytoplankton species have distinct colors, and consider reordering the way they are arranged in the figures. 
...
MB_Temp_DO <- ggarrange(MB_Temp, MB_DO, align="hv", ncol=1, nrow=2)

ggsave("MB_Temp_DO.tiff", plot=MB_Temp_DO, device="tiff", width=4, height=4, units="in", dpi=600)
...
MB_Temp_DO_SS <- ggarrange(MB_Temp, MB_DO, MB_SS, align="v", ncol=1, nrow=3)

ggsave("MB_Temp_DO_SS.tiff", plot=MB_Temp_DO_SS, device="tiff", width=4, height=6, units="in", dpi=600)
```
...
ggsave("SA_Temp_DO_SS_Wind_SF.tiff", plot=SA_Temp_DO_SS_Wind_SF, device="tiff", width=4, height=10, units="in", dpi=600)
...
This section is for other smaller analyses that would be required per the request of the reviewers.

## Variability in dissolved nutrients: 
 
Comment: “There is quite high variability in the dissolved nutrient data which is concerning given that no error was determined.”
 
```{r}
# Adding in method detection limits from https://www.seal-us.com/Methods/Discrete-Methods/Discrete-Analyzer-EPA-Methods
# We should double check this with somebody so we know what to use and what is correct. We can see if detection limits are posted in other manuscripts. 

DissNut_Stats <- read_xlsx("./DissolvedNutrients_2021.xlsx") %>%
  ...
  mutate(date = ymd(date),
        ...
        site = if_else(site == "Miss Bay", "Missisquoi Bay", "St. Albans Bay")) %>%
  mutate(NO3 = if_else(`NO3_mg/L` < 0.03, 0, `NO3_mg/L`),
         NH4 = if_else(`NH4_mg/L`< 0.004, 0, `NH4_mg/L`),
         PO4 = if_else(`PO4_mg/L` < 0.002, 0, `PO4_mg/L`),
         NO3 = NO3 * 1000,
         NH4 = NH4 * 1000,
         PO4 = PO4 * 1000,
         DIN = NO3 + NH4) %>%
...
PO4_SI <- DissNut_Stats %>%
    na.omit() %>%
  ggplot(aes(x = as.Date(date), y = PO4)) +
  geom_line(size = 0.5)+
  geom_pointrange(aes(ymin=PO4-PO4sd, ymax=PO4+PO4sd)) +
  theme_classic() +
  labs(y= expression(paste("PO4 (",~ mu, "g/L)")), x=NULL) +
   theme(plot.title = element_blank(),
         plot.subtitle = element_text(size=10, hjust=0, vjust = -0.5),
...
ggsave("PO4_SI.tiff", plot=PO4_SI , device="tiff", width=6, height=6, units="in", dpi=600)

NO3_SI <- DissNut_Stats %>%
    na.omit() %>%
  mutate(NO3 = NO3/1000) %>%
  ggplot(aes(x = as.Date(date), y = NO3)) +
  geom_line(size = 0.5)+
  geom_pointrange(aes(ymin=NO3-NO3sd, ymax=NO3+NO3sd)) +
  theme_classic() +
  labs(y="NO3 (mg/L)", x=NULL) +
   theme(plot.title = element_blank(),
         plot.subtitle = element_text(size=10, hjust=0, vjust = -0.5),
         axis.title = element_text(size=10), 
         axis.text =element_text(size=9),
...
ggsave("NO3_SI.tiff", plot=NO3_SI , device="tiff", width=6, height=6, units="in", dpi=600)

NH4_SI <- DissNut_Stats %>%
    na.omit() %>%
  mutate(NH4 = NH4/1000) %>%
  ggplot(aes(x = as.Date(date), y = NH4)) +
  geom_line(size = 0.5)+
  geom_pointrange(aes(ymin=NH4-NH4sd, ymax=NH4+NH4sd)) +
  theme_classic() +
  labs(y="NH4 (mg/L)", x=NULL) +
   theme(plot.title = element_blank(),
         plot.subtitle = element_text(size=10, hjust=0, vjust = -0.5),
         axis.title = element_text(size=10), 
         axis.text =element_text(size=9),
...
ggsave("NH4_SI.tiff", plot=NH4_SI , device="tiff", width=6, height=6, units="in", dpi=600)
...
“First, it seems like temperature was a pretty important predictor, and there has been a fair bit of work outlining the importance of temperature on cyanobacterial bloom formation as well as toxin production. I think more could have been made in the introduction about the role of temperature even though I realize it was not the mechanisms of interest you were hoping to highlight.”

Aim: Create a figure to examine the relationship and correlation between toxin concentrations and water temperature.
...
```
Toxin concentrations, when positive, typically increase with increasing water temperature. However, there are plenty of days with warm water that do not have positive hits for toxin concentrations. 

However, the HOTTEST water temperatures in St. Albans Bay did exhibit the HIGHEST concentrations of anatoxins. 
...
I don't think I need to further any figures for this, but mentioning it as I did in the discussion is okay. 
